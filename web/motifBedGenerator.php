...
$transcriptionFactors = explode(",", $transcriptionFactor);
...
if (count($transcriptionFactors) < 4){
	echo "track name=iRegulon_" . $feature . " description=\"iRegulon enchancer location for motif " . $feature . " and transcription factors:";
	$teller = 0;
	foreach ($transcriptionFactors as $tf){
		if ($teller == 0){
			echo " " . $tf;
			$teller = 1;
		}else{
	...
	echo "track name=iRegulon_" . $feature . " description=\"iRegulon enchancer location for motif " . $feature . "\" useScore=1" . "\n";
}
